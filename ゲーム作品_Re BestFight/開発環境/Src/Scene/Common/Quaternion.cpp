...
Quaternion::Quaternion(const Rot3DF& rad)
{
...
Quaternion Quaternion::Euler(const Rot3DF& rad)
{
...
Position3DF Quaternion::PosAxis(const Quaternion& q, const Position3DF& pos)
{
...
Position3DF Quaternion::PosAxis(const Position3DF& pos) const
{
    return PosAxis(Quaternion(w, x, y, z), pos);
}

Rot3DF Quaternion::ToEuler(const Quaternion& q)
{
...
Rot3DF Quaternion::ToEuler(void) const
{
...
Quaternion Quaternion::LookRotation(const Dir3DF& dir)
{
    Dir3DF up = { 0.0f, 1.0f, 0.0f };
    return LookRotation(dir, up);
}

Quaternion Quaternion::LookRotation(Dir3DF dir, Dir3DF up)
{

    dir = MyUtility::VNormalize(dir);
    Dir3DF right = MyUtility::VNormalize(VCross(up, dir));
    up = VCross(dir, right);
    auto m00 = right.x;
    auto m01 = right.y;
    auto m02 = right.z;
...
Quaternion Quaternion::GetRotation(MATRIX mat)
{
    ...
    {
        s = 0.5f / sqrtf(tr);
        ret.w = static_cast<double>(0.25f / s);
        ret.x = static_cast<double>((mat.m[1][2] - mat.m[2][1]) * s);
        ret.y = static_cast<double>((mat.m[2][0] - mat.m[0][2]) * s);
        ret.z = static_cast<double>((mat.m[0][1] - mat.m[1][0]) * s);
    }
    else
    {
        float max;
        ...
        if (max < mat.m[0][0])
        {
            ...
            float x = s * 0.5f;
            s = 0.5f / s;
            ret.x = static_cast<double>(x);
            ret.y = static_cast<double>((mat.m[0][1] + mat.m[1][0]) * s);
            ret.z = static_cast<double>((mat.m[2][0] + mat.m[0][2]) * s);
            ret.w = static_cast<double>((mat.m[1][2] - mat.m[2][1]) * s);
        }
        else
            if (max == mat.m[1][1])
            {
                ...
                float y = s * 0.5f;
                s = 0.5f / s;
                ret.x = static_cast<double>((mat.m[0][1] + mat.m[1][0]) * s);
                ret.y = static_cast<double>(y);
                ret.z = static_cast<double>((mat.m[1][2] + mat.m[2][1]) * s);
                ret.w = static_cast<double>((mat.m[2][0] - mat.m[0][2]) * s);
            }
            else
            {
                ...
                float z = s * 0.5f;
                s = 0.5f / s;
                ret.x = static_cast<double>((mat.m[2][0] + mat.m[0][2]) * s);
                ret.y = static_cast<double>((mat.m[1][2] + mat.m[2][1]) * s);
                ret.z = static_cast<double>(z);
                ret.w = static_cast<double>((mat.m[0][1] - mat.m[1][0]) * s);
            }
    }
...
Dir3DF Quaternion::GetDir(const Dir3DF& dir) const
{
    return PosAxis(dir);
}

Dir3DF Quaternion::GetForward(void) const
{
    return GetDir(MyUtility::DIR_F);
}

Dir3DF Quaternion::GetBack(void) const
{
    return GetDir(MyUtility::DIR_B);
}

Dir3DF Quaternion::GetRight(void) const
{
    return GetDir(MyUtility::DIR_R);
}

Dir3DF Quaternion::GetLeft(void) const
{
    return GetDir(MyUtility::DIR_L);
}

Dir3DF Quaternion::GetUp(void) const
{
    return GetDir(MyUtility::DIR_U);
}

Dir3DF Quaternion::GetDown(void) const
{
...
//float
inline double NORM(float a, float b, float c, float d) {
    return sqrt(a * a + b * b + c * c + d * d);
}

Quaternion Quaternion::FromToRotation(const Dir3DF& fromDir, const Dir3DF& toDir)
{
    Rot3DF axis = VCross(fromDir, toDir);
	double angle = MyUtility::AngleDeg(fromDir, toDir);
	if (angle >= 179.9196)
	{
...
{
    return (q1.x == q2.x && q1.y == q2.y && q1.z == q2.z && q1.w == q2.w) ? true : false;
}
