...
class Fighter : public CharactorBase
{
public:

	Fighter(std::weak_ptr<Camera> camera);
	~Fighter()override;
...
private:
	...
	static constexpr float DEFENCE_MAX = 1.5f;//デバッグ用
	static constexpr float SKILL_DEFAULT_MAX = 100.0f;//デバッグ用
	static constexpr float SKILL_MAX = 100.0f;//デバッグ用
	static constexpr float AVOID_POWER = 25.0f;//デバッグ用
	static constexpr float ATTACK_POWER = 1.5f;
