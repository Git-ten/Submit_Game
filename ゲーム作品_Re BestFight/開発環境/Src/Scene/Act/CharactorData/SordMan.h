...
class SordMan : public CharactorBase
{
public:

	SordMan(std::weak_ptr<Camera> camera);
	~SordMan()override;
...
private:
	...
	static constexpr float DEFENCE_MAX = 2.0f;//デバッグ用
	static constexpr float SKILL_DEFAULT_MAX = 100.0f;//デバッグ用
	static constexpr float SKILL_MAX = 100.0f;//デバッグ用
	static constexpr float AVOID_POWER = 45.0f;//デバッグ用
	static constexpr float ATTACK_POWER = 1.0f;
	static constexpr float FORWARD_ATTACK_AREA = 230.0f;//デバッグ用
	static constexpr float SPEED = 50.0f;//速度

	static constexpr Position3DF START_SKILL_CAMERA_POS = {30.0f, 100.0f, 350.0f };
	static constexpr Position3DF END_SKILL_CAMERA_POS = { -800.0f, 50.0f, 1000.0f };
	static constexpr Position3DF FINISH_WEEK_CAMERA_POS = { 0.0f, 130.0f, -300.0f };
	static constexpr Position3DF FINISH_STRONG_CAMERA_POS = { 0.0f, 500.0f, -850.0f };
	static constexpr Color3D MY_COLOR = { 0.0f, 0.0f, 1.0f };
