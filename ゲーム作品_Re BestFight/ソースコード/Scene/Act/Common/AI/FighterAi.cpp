...
FighterAi::FighterAi(std::weak_ptr<ActorBase> actor) : ActorAi(actor)
{
...
FighterAi::~FighterAi(void)
{
}

void FighterAi::ActionComandWeek_1(void)
{
...
void FighterAi::ActionComandWeek_2(void)
{
...
void FighterAi::ActionComandWeek_3(void)
{
...
void FighterAi::ActionComandWeek_4(void)
{
	...
	else
	{
		...
		if (!comandFlag_ && actor_.lock()->animController_->GetStepAnimTime() >= 5.0f)
		{
			...
			actor_.lock()->SetSwayVector(VScale(actor_.lock()->transform_->GetForwardLocal(), 90.0f));
			actor_.lock()->SetJumpVector({ 0.0f, 18.0f, 0.0f });
			//空振りエフェクト
			//空振りといっても攻撃が当たった場合でも再生される
			//エフェクト
...
void FighterAi::ActionComandWeek_5(void)
{
	...
	else
	{
		...
		if (!comandFlag_ && actor_.lock()->animController_->GetStepAnimTime() >= 30.0f)
		{
			//デバッグ用の攻撃フラグ
			comandFlag_ = true;
			//攻撃行動による移動処理
			...
			//actor_.lock()->transform_->pos = VAdd(actor_.lock()->transform_->pos, VScale(dir, 320.0f));
			actor_.lock()->SetSwayVector(VScale(actor_.lock()->transform_->GetForwardLocal(), 70.0f));
			actor_.lock()->SetJumpVector({ 0.0f, 30.0f, 0.0f });
			//空振りエフェクト
			//空振りといっても攻撃が当たった場合でも再生される
			//エフェクト
			Position3DF effPos = actor_.lock()->GetPlayerForwardPos(200.0f);
...
void FighterAi::ActionComandWeek_6(void)
{
	...
	else
	{
		...
		if (!comandFlag_ && actor_.lock()->animController_->IsPlayAnim())
		{
			//デバッグ用の攻撃フラグ
			comandFlag_ = true;
			//空振りエフェクト
		...
		//攻撃モーションが終了し、地面に足が着くまで処理が行われない
		else if (!actor_.lock()->animController_->IsPlayAnim())
		{
			actor_.lock()->actorCam_->ChangeDefaultMode();
			if (!comandFlag_ && actor_.lock()->IsCollGroundFlag())
			{
				//アクションを終了
				int nSearch = SearchNodeIdx(nowIndex_);
				if (nSearch >= 0)
			...
			else if (comandFlag_ && actor_.lock()->IsCollGroundFlag())
			{
				atackWeakCnt_ = 0;//攻撃回数をリセット
				atackStrongCnt_ = 0;
				comandFlag_ = false;
				//上にアッパーする
				actor_.lock()->SetJumpVector({ 0.0f, 40.0f, 0.0f });
			}
		}
	}
}

void FighterAi::ActionComandStrong_1(void)
{
	...
	else
	{
		//当たり判定の発生
		//アニメーションのステップタイムで判断する
		if (actor_.lock()->animController_->GetStepAnimTime() >= 15.0f && !colliderFlag_ &&
			actor_.lock()->enemy_.lock()->GetState() != ActorBase::STATE::UNLEASH_SKILL &&
			actor_.lock()->enemy_.lock()->GetState() != ActorBase::STATE::QUICK_MOVE &&
			actor_.lock()->enemy_.lock()->GetState() != ActorBase::STATE::COUNTER_ATTACK &&
			actor_.lock()->enemy_.lock()->actAi_->GetNowIndex() != INDEX_CON_COMAND_COUNTERATTACK)
		{
			...
			if (actor_.lock()->animController_->GetStepAnimTime() >= 30.0f &&
				actor_.lock()->animController_->GetStepAnimTime() < 32.0f)
			{
				twoAttack = true;
				canCollFlag = true;
			}
...
void FighterAi::ActionComandStrong_2(void)
{
	...
	else
	{
		//当たり判定の発生
		//アニメーションのステップタイムで判断する
		if (actor_.lock()->animController_->GetStepAnimTime() >= 27.0f && !colliderFlag_ &&
			actor_.lock()->enemy_.lock()->GetState() != ActorBase::STATE::UNLEASH_SKILL &&
			actor_.lock()->enemy_.lock()->GetState() != ActorBase::STATE::QUICK_MOVE &&
			actor_.lock()->enemy_.lock()->GetState() != ActorBase::STATE::COUNTER_ATTACK &&
			actor_.lock()->enemy_.lock()->actAi_->GetNowIndex() != INDEX_CON_COMAND_COUNTERATTACK)
...
void FighterAi::ActionComandStrong_3(void)
{
	float dis = VSquareSize(VSub(actor_.lock()->enemy_.lock()->transform_->pos,
		actor_.lock()->transform_->pos));
	//距離によりカメラモードを切り替える
	...
	else
	{
		//当たり判定の発生
		//アニメーションのステップタイムで判断する
		if (actor_.lock()->animController_->GetStepAnimTime() >= 75.0f && !colliderFlag_ &&
			actor_.lock()->enemy_.lock()->GetState() != ActorBase::STATE::UNLEASH_SKILL &&
			actor_.lock()->enemy_.lock()->GetState() != ActorBase::STATE::QUICK_MOVE &&
			actor_.lock()->enemy_.lock()->GetState() != ActorBase::STATE::COUNTER_ATTACK &&
			actor_.lock()->enemy_.lock()->actAi_->GetNowIndex() != INDEX_CON_COMAND_COUNTERATTACK)
...
void FighterAi::ActionComandUnleashSkill(void)
{	//二回以上連続をそれぞれ別々に判定するためのフラグ
	...
	static constexpr int ANIM_TIME_MAX = 9;
	...
	if (actor_.lock()->GetState() != ActorBase::STATE::UNLEASH_SKILL)
	{
		...
		actor_.lock()->PlayEffect(EffectManager::EFF::SKILL_FLAME, ActorBase::USE_EFFECT::SKILL_FLAME, effPos, effRot);
		...
		//必殺技音
		actor_.lock()->SoundPlay(SoundManager::SOD::SKILL_FLAME, DX_PLAYTYPE_BACK);
	}
	else
	{
		//自身のフレーム速度
			...
			actor_.lock()->enemy_.lock()->GetState() != ActorBase::STATE::UNLEASH_SKILL)
		{
			...
			for (int ii = 1; ii < actor_.lock()->GetAttackArea(); ii++)
			{
				...
				{
					float power = actor_.lock()->GetAttackPower();
					actor_.lock()->enemy_.lock()->userInterface_->AddDamagePower(power + 2.0f);
					...
					{
						actor_.lock()->enemy_.lock()->SetDamageMotion([&]() {actor_.lock()->DamageBlowStagger(); });
						actor_.lock()->enemy_.lock()->SetSwayVector(VScale(actor_.lock()->transform_->GetForwardLocal(), 130.0f));
						actor_.lock()->enemy_.lock()->SetJumpVector({ 0.0f, 80.0f, 0.0f });
						collCnt = 0;
						canCollFlag = false;
						//当たり判定フラグ
						colliderFlag_ = true;
					...
					actor_.lock()->PlayEffect(EffectManager::EFF::LASER_FIRE, ActorBase::USE_EFFECT::LASER_FIRE, effPos, effRot);
					actor_.lock()->SoundPlay(SoundManager::SOD::ATTACK_STRONG, DX_PLAYTYPE_BACK);
					break;
				}
			}
		...
		if (!comandFlag_ && actor_.lock()->animController_->GetStepAnimTime() >= 55.0f)
		{
			//デバッグ用の攻撃フラグ
			comandFlag_ = true;
			//攻撃行動による移動処理
